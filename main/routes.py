...
def main():
    ...
    if search_form.validate_on_submit():
        ...
        free_shipping = int(search_form.free_shipping.data[0]) if len(search_form.free_shipping.data) != 0 else None 
        ...
        fetch_and_save_data(search_parameter=search_parameter, delivery_destination=delivery_destination, free_shipping=free_shipping, limit=limit, market=market, sort_by=sort_by, min_price=min_price, max_price=max_price, conditions_id_list=conditions_id_list, currency=currency)
...
def delete_item(id: int):
    ...
    average_price, median_price, min_price, max_price = format_query_price_information(price_list)
